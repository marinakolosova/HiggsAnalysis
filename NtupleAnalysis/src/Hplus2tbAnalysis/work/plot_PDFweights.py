...
USAGE:
./plot_PDFweights.py -m <pseudo_mcrab> [opts]

To run the QCD scales: 
./plot_PDFweights.py --url -m Hplus2tbAnalysis_Weight_Nominal,Hplus2tbAnalysis_Weight_1,Hplus2tbAnalysis_Weight_2,Hplus2tbAnalysis_Weight_3,Hplus2tbAnalysis_Weight_4,Hplus2tbAnalysis_Weight_6,Hplus2tbAnalysis_Weight_8

To run the PDF uncertainties:
./plot_PDFweights.py --url -m Hplus2tbAnalysis_Weight_Nominal,Hplus2tbAnalysis_Weight_9,Hplus2tbAnalysis_Weight_10,Hplus2tbAnalysis_Weight_12,Hplus2tbAnalysis_Weight_16,Hplus2tbAnalysis_Weight_23,Hplus2tbAnalysis_Weight_27,Hplus2tbAnalysis_Weight_32,Hplus2tbAnalysis_Weight_46,Hplus2tbAnalysis_Weight_59,Hplus2tbAnalysis_Weight_63,Hplus2tbAnalysis_Weight_77,Hplus2tbAnalysis_Weight_80,Hplus2tbAnalysis_Weight_96
...
def GetHistoKwargs(h, opts):
    _moveLegend =  {"dx": -0.08, "dy": -0.02, "dh": 0.13}
    logY    = True
    _yLabel = "Events / %.0f "
    yMin    = 1e0
    if logY:
    ...
    kwargs["rebinX"] = systematics._dataDrivenCtrlPlotBinning["LdgTetrajetMass_AfterAllSelections"]
...
def PlotHistos(datasetName, hPath):
    ...
    while j < len(opts.mcrabs):
       ...
       # Set/Overwrite cross-sections
        ...
        if (j == 0):
            # First given multicrab - nominal
            refHisto = histograms.Histo(histo,  "nominal",     legendStyle = "LP", drawStyle="HIST") #fix legend
        else:
            
            text = str(j)
            histoList.append(histograms.Histo(histo, "QCD scale "+textweight,     legendStyle = "LP", drawStyle="HIST")) #fix legend
        j = j + 1
    ...
    saveName = "NewNormalized_QCDscales_"+hPath.split("/")[-1]
...
if __name__ == "__main__":
    ...
    if not opts.batchMode:
        raw_input("=== plot_PDFweights.py: Press any key to quit ROOT ...")
