...
def main(opts):
    ...
    for opt in optModes:
        ...
        # Remove datasets
        removeList = ["QCD-b"]#, "Charged"] #Marina
        if not opts.useMC:
...
def GetBinLabels(region, histoPaths):
    ...
    for h in histoPaths:
        ...
        if region not in h:
            ...
            print "h = ", h, "    h.split(region)[-1] = ", h.split(region)[-1]
    return binLabels
...
def PlotHistosAndCalculateTF(datasetsMgr, histoList, binLabels, opts):
    ...
    if opts.altPlot:
        ...
        # Marina
        # Add the Signal
        p2 = plots.DataMCPlot(datasetsMgr,  'ForFakeBMeasurement/QGLR_SR/QGLR_SRInclusive',  saveFormats=[])
        
        hSignal_800 = p2.histoMgr.getHisto('ChargedHiggs_HplusTB_HplusToTB_M_800').getRootHisto()
        hhSignal_800= histograms.Histo(hSignal_800, 'ChargedHiggs_HplusTB_HplusToTB_M_800', "H^{+} m_{H^+}=800 GeV")#plots._legendLabels['ChargedHiggs_HplusTB_HplusToTB_M_800'])
        ...
        sampleS = "ChargedHiggs_HplusTB_HplusToTB_M_800"
        hSignif_250 = p.histoMgr.getHisto(sampleS).getRootHisto().Clone(sampleS)
        hSignif_250.Reset()
        
        hSignif_500 = p.histoMgr.getHisto(sampleS).getRootHisto().Clone(sampleS)
        hSignif_500.Reset()

        hSignif_800 = p.histoMgr.getHisto(sampleS).getRootHisto().Clone(sampleS)
        hSignif_800.Reset()

        hSignif_1000 = p.histoMgr.getHisto(sampleS).getRootHisto().Clone(sampleS)
        hSignif_1000.Reset()
        
        hBkg = p2.histoMgr.getHisto(sampleS).getRootHisto().Clone("Bkg")
        hBkg.Reset()
        
        print "Fake-B Histogram =", hFakeB.getRootHisto().GetNbinsX()
        # Bkg: FakeB + Genuine B
        ...
        for i in range (1, nBins+1):
            ...
            signif_250 = stat.significance(s_250, b, sigmaB, option="Asimov")
            signif_500 = stat.significance(s_500, b, sigmaB, option="Asimov")
            signif_800 = stat.significance(s_800, b, sigmaB, option="Asimov")
            signif_1000 = stat.significance(s_1000, b, sigmaB, option="Asimov")
